...
def update_product(product_id):
    ...
    product = Product.query.join(Category,Category.categoryID == Product.categoryID) \
               .add_columns(Product.productID,Product.productName, Product.categoryID, Product.description, Product.image, Product.price, Product.quantity, Category.categoryName).first()
    ...
    if form.validate_on_submit():
        ...
        product1 = Product.query.filter(Product.productID == product_id).first()
        product1.productName = form.productName.data
        product1.categoryID = form.categoryName.data
        product1.price = form.price.data
        product1.description = form.description.data
        product1.quantity = form.quantity.data
...
@app.route("/category_home/<category_id>/update", methods=['GET', 'POST'])
@login_required
def update_category(category_id):
    ...
    if form.validate_on_submit():          # notice we are are not passing the nickname from the form
        ...
        return redirect(url_for('category_home'))
    elif request.method == 'GET':                # notice that we ARE passing the nickname to the form
